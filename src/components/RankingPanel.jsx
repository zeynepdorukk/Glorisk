...
export default function RankingPanel({
  ...
  const { copy, regionLabel, countryName } = useI18n();
  const sorts = useMemo(() => ({
    riskiest: { label: copy.ranking.riskiest, compare: (a, b) => b.score - a.score, icon: ArrowDownWideNarrow },
    safest: { label: copy.ranking.safest, compare: (a, b) => a.score - b.score, icon: ArrowUpNarrowWide },
  }), [copy.ranking.riskiest, copy.ranking.safest]);
  ...
  }, [countries, region, bandFilter, sort, sorts]);
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="space-y-3 border-b border-white/10 p-4">
        ...
        >
          <Search size={15} />
          <span className="flex-1 text-left">{copy.ranking.search}</span>
        ...
        <div className="flex gap-2">
          <select
            ...
            aria-label={copy.ranking.regionFilter}
          ...
          >
            <option value="all">{copy.ranking.allRegions}</option>
            {regions.map((name) => (
          ...
          <button
            ...
            aria-label={copy.ranking.sortedBy(sorts[sort].label)}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto" role="listbox" aria-label={copy.ranking.aria}>
        {rows.length === 0 && <p className="p-6 text-center text-sm text-neutral-500">{copy.ranking.empty}</p>}
        {rows.map((country) => {
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                <span className="flex items-center gap-1.5">
                  <span className="truncate text-sm text-white">{countryName(country)}</span>
                  {country.confidence === 'low' && (
                    <span title={copy.ranking.limited} className="shrink-0 text-[10px] text-amber-400">
                      ●
                    </span>
                  )}
                </span>
                <span className="block truncate text-[11px] text-neutral-500">{regionLabel(country.region)}</span>
              </span>
              <Sparkline
                ...
                width={48}
                height={18}
                ariaLabel={copy.ranking.trend(countryName(country))}
              />
              <span
