  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex items-center gap-2">
        <span className="font-mono text-[11px] text-sky-400">{value}% {copy.weight.eco}</span>
        <input
          id="pillar-weight"
          type="range"
          min={0}
          ...
          aria-valuetext={copy.weight.value(value, 100 - value)}
          className="h-1 w-28 cursor-pointer appearance-none rounded-full bg-gradient-to-r from-sky-500 to-violet-500 accent-white"
        />
        <span className="font-mono text-[11px] text-violet-400">{100 - value}% {copy.weight.gov}</span>
      </div>
    </div>
  );
}
